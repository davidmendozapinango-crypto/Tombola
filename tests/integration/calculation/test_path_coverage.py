...
def test_single_path_coverage():
    ...
    command = make_command(actor_id='player-001', operation_key='sum', path_context={'mode': 'single'}, input_payload={'a': 2, 'b': 4}, ui_origin='screen')
...
def test_batch_path_coverage():
    engine_context = _make_engine()
    command = make_command(actor_id='player-001', operation_key='sum', path_context={'mode': 'batch'}, input_payload={'items': [1, 2, 3]}, ui_origin='screen')
    result = execute_command(command, engine_context)
    assert result['status'] == 'success'
...
def test_tombola_card_coverage_path():
    ...
    command = make_command(actor_id='player-001', operation_key='tombola', path_context={'domain': 'tombola', 'path': 'card_coverage'}, input_payload={'card': card, 'drawn_numbers': drawn_numbers}, ui_origin='screen')
    result = execute_command(command, engine_context)
    assert result['status'] == 'success'
...
def test_tombola_game_score_path():
    ...
    command = make_command(actor_id='player-001', operation_key='tombola', path_context={'domain': 'tombola', 'path': 'game_score'}, input_payload={'main_card': main_card, 'complement_card': complement_card, 'drawn_numbers': drawn_numbers}, ui_origin='screen')
    result = execute_command(command, engine_context)
    assert result['status'] == 'success'
...
def test_tombola_sdg_context_path():
    engine_context = _make_engine()
    command = make_command(actor_id='player-001', operation_key='tombola', path_context={'domain': 'tombola', 'path': 'sdg_context'}, input_payload={'sdg_id': 4}, ui_origin='screen')
    result = execute_command(command, engine_context)
    assert result['status'] == 'success'
