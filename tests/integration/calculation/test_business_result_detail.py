...
def test_result_includes_business_meaningful_value():
    screen = _make_screen()
    raw_payload = {'actor_id': 'player-001', 'operation_key': 'sum', 'path_context': {'mode': 'single'}, 'input_payload': {'a': 7, 'b': 8}, 'ui_origin': 'calculator_screen'}
    result = on_calculate_action(screen, raw_payload)
    assert result['status'] == 'success'
    assert result['path_id'] == 'sum:single'
    assert 'result_payload' in result
    assert result['result_payload'].get('sum') == 15
    assert result['result_payload'].get('operands') == [7, 8]