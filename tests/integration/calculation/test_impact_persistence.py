...
def test_multiple_executions_produce_valid_loadable_records(tmp_path):
    ...
    for index in range(3):
        command = make_command(actor_id=f'player-{index:03d}', operation_key='sum', path_context={'mode': 'single'}, input_payload={'a': index, 'b': index + 1}, ui_origin='calculator_screen')
...
def test_malformed_persistence_lines_are_skipped(tmp_path):
    ...
    file_path.write_text('{"impact_id": "good-1", "interaction_point": "x", "before_behavior": "b", "after_behavior": "a", "validation_reference": "v"}\nthis is not json\n{"impact_id": "good-2", "interaction_point": "x", "before_behavior": "b", "after_behavior": "a", "validation_reference": "v"}\n')
