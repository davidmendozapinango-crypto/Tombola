...
def registry():
    reg = make_registry()
    register_rule(reg, make_rule(path_id='sum:single', priority=10, entry_conditions=[_mode_single], output_definition={'impact_marker': 'sum_single'}))
    register_rule(reg, make_rule(path_id='sum:batch', priority=5, entry_conditions=[_mode_batch], output_definition={'impact_marker': 'sum_batch'}))
    return reg
...
def test_duplicate_priority_per_operation_is_rejected(registry):
    with pytest.raises(ValueError, match='Duplicate priority'):
        register_rule(registry, make_rule(path_id='sum:extra', priority=10, entry_conditions=[lambda ctx, payload: ctx.get('mode') == 'extra'], output_definition={'impact_marker': 'sum_extra'}))