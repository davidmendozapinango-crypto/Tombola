"""Pruebas para los ayudantes de generación y marcado de tarjetas.

Descripción:
    Estas pruebas verifican la generación correcta de tarjetas NxN, el
    comportamiento de marcado de números y el cálculo de puntos/sumas.

Notas:
    - Los comentarios adicionales ayudan a usuarios principiantes a entender
      qué se verifica en cada prueba.
"""
...
def test_mark_number_detects_present_value():
    # Tarjeta pequeña 2x2 con valores presentes
    card = [[1, 2], [3, 4]]
    marked = set()
...
def test_mark_number_ignores_missing_value():
    # Si se marca un número que no existe no debe añadirse al conjunto
    card = [[1, 2], [3, 4]]
    marked = set()
