...
def main() -> None:
    ...
    while state["running"]:
        ...
        if handler is None:
            # No handler available for the current screen: request the
            # application to stop running. The original code used `break` to
            # exit the loop; here we set the `running` flag to False and
            # continue so the while loop will terminate naturally.
            state["running"] = False
            continue
