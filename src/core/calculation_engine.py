"""Orquestador de cálculos con manejo determinista de fallos (no OOP).

Este módulo coordina la ejecución interna de comandos originados desde la
interfaz (GUI). Valida precondiciones, comprueba dependencias, ejecuta la
regla de cálculo correspondiente y registra el impacto observable para
auditoría.
"""
...
def make_engine_context(
    ...
    precondiciones, comprobador de dependencias, catálogo de errores y
    almacenamiento de impacto.
    """
    return {
        "registry": registry,
...
def execute_command(command: Dict[str, Any], context: Dict[str, Any]) -> Dict[str, Any]:
    ...
    Flujo general
    ------------
    1. Verificar que el comando proviene de la UI (`ui_origin`).
    2. Validar campos obligatorios mediante `validate_required_fields`.
    3. Seleccionar la regla aplicable con `select_rule`.
    4. Chequear dependencias bloqueantes llamando a `check_dependency`.
    5. Evaluar precondiciones con `evaluate_preconditions`.
    6. Calcular el resultado con `compute_result` y registrar impacto.

    Devuelve
    -------
    Dict[str, Any]
        En caso de éxito: `{'status': 'success', 'path_id': ..., 'result_payload': ..., 'impact_marker': ...}`.
        En caso de fallo: respuesta de error construida por
