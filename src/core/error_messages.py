...
def get_message(message_id: str, locale: str=DEFAULT_LOCALE, catalog: Optional[Dict[str, Dict[str, str]]]=None) -> str:
    """Look up a localized message by id."""
    catalog = catalog or MESSAGES
    message_catalog = catalog.get(message_id, {})
    return message_catalog.get(locale) or message_catalog.get(DEFAULT_LOCALE) or message_id

def register_message(message_id: str, text: str, locale: str=DEFAULT_LOCALE, catalog: Optional[Dict[str, Dict[str, str]]]=None) -> None:
...
def build_error_response(error_type: str, error_code: str, catalog: Optional[Dict[str, Dict[str, str]]]=None, locale: str=DEFAULT_LOCALE) -> Dict[str, str]:
    """Build a standardized failure response dictionary."""
    return {'status': 'failure', 'error_type': error_type, 'error_code': error_code, 'error_message': get_message(error_code, locale, catalog), 'retry_hint': 'Verifique los datos e intente nuevamente.'}