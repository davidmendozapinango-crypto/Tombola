"""Familia H: diagonales excepto la fila central (principal y complementaria idénticas).

Descripción:
    Incluye las dos diagonales pero excluye la fila central para crear una
    figura con hueco horizontal en el centro. Tanto la versión principal
    como la complementaria coinciden.
"""
...
def mask_main(n: int) -> "np.ndarray":
    """Máscara pública que devuelve las diagonales excepto la fila central."""
    _validate_n(n)
...
def _assign_random_permutation_to_mask(
    ...
    """Asignar permutación a celdas ocupadas; opcionalmente reproducible."""
    n = mask.shape[0]
