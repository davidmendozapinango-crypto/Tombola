"""Familia B: banda por columnas (principal) y por filas (complemento).

Descripción:
    Analogía con la Familia A pero intercambiando los roles de principal y
    complementario. Proporciona generadores numerados y shims de máscara
    por compatibilidad.
"""
...
def _mask_main(n: int) -> "np.ndarray":
    """Máscara principal: banda por columnas."""
    _validate_n(n)
...
def _mask_complement(n: int) -> "np.ndarray":
    """Máscara complementaria: banda por filas."""
    _validate_n(n)
    mat = np.zeros((n, n), dtype=int)
    for i in range(n):
        left = min(i, n - 1 - i)
