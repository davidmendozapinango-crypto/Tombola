...
from src.ui.common import draw_button, draw_error_message, draw_message_panel, draw_text, get_font

def _layout() -> Dict[str, pygame.Rect]:
    """Return the UI rectangles for the game screen."""
    return {'draw': pygame.Rect(WINDOW_WIDTH // 2 - 100, 120, 200, 45), 'simulate': pygame.Rect(WINDOW_WIDTH // 2 - 100, 175, 200, 45), 'result': pygame.Rect(WINDOW_WIDTH // 2 - 100, 120, 200, 45), 'menu': pygame.Rect(WINDOW_WIDTH // 2 - 100, 640, 200, 45), 'victory': pygame.Rect(WINDOW_WIDTH // 2 - 150, 540, 300, 45)}
...
def _draw_card(surface: pygame.Surface, card: Any, marked: Any, top_left: Any, cell_size: int, title: str, sdg_color: Any) -> None:
    ...
    for (row_index, row) in enumerate(card):
        for (col_index, value) in enumerate(row):
            rect = pygame.Rect(top_left[0] + col_index * cell_size, top_left[1] + row_index * cell_size, cell_size, cell_size)
            if value is None:
            ...
            draw_text(surface, str(value), rect.center, font_size=max(12, cell_size // 2), color=text_color, center=True)
...
def _draw_number(state: Dict[str, Any]) -> str:
    ...
    winner = check_winner(main_card, session['marked_main'], complement_card, session['marked_complement'], card_type)
    if winner:
...
def _save_game(state: Dict[str, Any]) -> None:
    ...
    if player is None:
        return
    game = make_game_record(player_id=player['player_id'], sdg_id=session.get('sdg_id', 1), dimension=session.get('dimension', 5), main_card=session['main_card'], complement_card=session['complement_card'], drawn_numbers=session['drawn_numbers'][:])
...
def _draw_trophy_icon(surface: pygame.Surface, rect: pygame.Rect) -> None:
    ...
    (cx, cy) = rect.center
    pygame.draw.polygon(surface, COLOR_PINE, [(cx, cy - 14), (cx + 10, cy - 6), (cx + 8, cy + 4), (cx - 8, cy + 4), (cx - 10, cy - 6)])
    pygame.draw.rect(surface, COLOR_PINE, (cx - 3, cy + 4, 6, 8))
    pygame.draw.rect(surface, COLOR_PINE, (cx - 6, cy + 11, 12, 3))

def _draw_victory_modal(surface: pygame.Surface, state: Dict[str, Any], rects: Dict[str, pygame.Rect], hovered: Dict[str, bool], focused: str) -> None:
    ...
    modal_w = 520
    modal_h = 520
    modal_rect = pygame.Rect((WINDOW_WIDTH - modal_w) // 2, (WINDOW_HEIGHT - modal_h) // 2, modal_w, modal_h)
    ...
    draw_text(surface, '¡RESULTADO GLOBAL DE LA PARTIDA!', (modal_rect.centerx, modal_rect.y + 95), font_size=13, color=COLOR_PINE, center=True)
    draw_text(surface, '¡GANADOR!', (modal_rect.centerx, modal_rect.y + 125), font_size=42, color=COLOR_PINE, center=True)
    ...
    stats = [('IDENTIDAD', card_label), ('SUMA DE CELDAS', str(total_sum)), ('PUNTAJE ODS', f'+{points} pts')]
    col_width = (modal_w - 80) // 3
    for (index, (label, value)) in enumerate(stats):
        x = table_rect.x + index * col_width + col_width // 2
        draw_text(surface, label, (x, table_rect.y + 12), font_size=10, color=COLOR_CHARCOAL, center=True)
        draw_text(surface, value, (x, table_rect.y + 36), font_size=16, color=COLOR_PINE, center=True)
        if index < 2:
            pygame.draw.line(surface, COLOR_SAGE_LIGHT, (table_rect.x + (index + 1) * col_width, table_rect.y + 10), (table_rect.x + (index + 1) * col_width, table_rect.bottom - 10), 1)
    ...
    for quote_line in quote_lines[:2]:
        ...
        surface.blit(rendered, rect)
        quote_line_y += 14
    draw_text(surface, 'Autorespaldo consolidado exitosamente en JUEGOS.bin e historial del jugador.', (modal_rect.centerx, quote_rect.bottom + 25), font_size=10, color=COLOR_CHARCOAL, center=True)
    draw_button(surface, 'IR A MENÚ', rects['victory'], hovered=hovered['victory'], focused=focused == 'victory')
...
def draw(surface: pygame.Surface, state: Dict[str, Any]) -> None:
    ...
    sdg_name = get_sdg_name(sdg_id)
    draw_text(surface, f'Partida - {sdg_name}', (WINDOW_WIDTH // 2, 30), font_size=32, color=COLOR_PINE, center=True)
    left_card_pos = (80, 180)
    _draw_card(surface, session['main_card'], session['marked_main'], left_card_pos, cell_size, f'Principal - {sdg_name}', COLOR_PINE)
    right_card_pos = (WINDOW_WIDTH - 80 - dimension * cell_size, 180)
    _draw_card(surface, session['complement_card'], session['marked_complement'], right_card_pos, cell_size, f'Complemento - {sdg_name}', COLOR_PINE)
    ...
    if session.get('game_over'):
        ...
        last_text = f'Ultimo numero: {last}' if last is not None else 'Presione Sacar numero'
        draw_text(surface, last_text, (WINDOW_WIDTH // 2, 90), font_size=24, color=COLOR_CHARCOAL, center=True)
        draw_button(surface, 'Sacar numero', rects['draw'], hovered=hovered['draw'], focused=focused == 'draw')
        draw_button(surface, 'Simular completo', rects['simulate'], hovered=hovered['simulate'], focused=focused == 'simulate')
    draw_text(surface, f"Numeros sorteados: {len(session['drawn_numbers'])}", (WINDOW_WIDTH // 2, 620), font_size=20, color=COLOR_CHARCOAL, center=True)
    draw_button(surface, 'Menu', rects['menu'], hovered=hovered['menu'], focused=focused == 'menu')
    if state.get('error_message'):
        draw_error_message(surface, state['error_message'], (WINDOW_WIDTH // 2, WINDOW_HEIGHT - 100), font_size=20)
