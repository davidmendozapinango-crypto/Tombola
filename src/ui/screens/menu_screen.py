...
def _draw_medal_icon(surface: pygame.Surface, rect: pygame.Rect) -> None:
    ...
    pygame.draw.polygon(
        surface,
        COLOR_PINE,
        [(cx - 8, cy + 8), (cx - 4, cy + 8), (cx - 6, cy + 18)],
    )
    pygame.draw.polygon(
        surface,
        COLOR_PINE,
        [(cx + 4, cy + 8), (cx + 8, cy + 8), (cx + 6, cy + 18)],
    )
...
def _draw_header(surface: pygame.Surface, state: Dict[str, Any]) -> None:
    ...
    # Subtitle with highlighted state name (split in two lines)
    ...
    line2 = (
        "impulsa la consciencia eco-social indispensable para el cumplimiento"
        " del Plan Patria Sostenible."
    )
