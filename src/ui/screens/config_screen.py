"""
Pantalla de configuracion de cartones (no-OOP).

Descripción:
  Permite seleccionar la dimension del cartón y el tema ODS antes de iniciar
  una partida. Genera las figuras (patrones) y crea los cartones en la sesión
...
def _layout() -> Dict[str, pygame.Rect]:
    """
    Devuelve los rectángulos (posiciones) usados por la pantalla de
    configuracion.
    """
...
def _dimensions() -> list[int]:
    """Dimensiones soportadas para los cartones (lista de N)."""
    return [5, 7, 9, 11, 13, 15]


def init_config(state: Dict[str, Any]) -> None:
    """
    Inicializa el estado para la pantalla de configuracion con valores por
    defecto (dimension=5, sdg_id=1) y rectangulos/enfoque.
    """
...
def _change_dimension(state: Dict[str, Any], direction: int) -> None:
    """Ajusta la dimension seleccionada dentro de las opciones disponibles."""
...
def _change_sdg(state: Dict[str, Any], direction: int) -> None:
    """Cicla entre los IDs de ODS disponibles segun la direccion indicada."""
...
def _start_game(state: Dict[str, Any]) -> str:
    """
    Genera los cartones (principal y complemento) usando el tipo de figura
    correspondiente al ODS seleccionado y almacena los cartones en la sesion.

    Devuelve:
        str: Nombre de la pantalla a mostrar ('card_display').
    """
...
def _activate(state: Dict[str, Any], name: str) -> str:
    """Activar un control en esta pantalla.

    Traduce la accion del control (flechas, continuar, volver) a efectos de UI.
    """
    if name == "dim_left":
