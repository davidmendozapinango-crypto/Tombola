...
def _layout() -> Dict[str, pygame.Rect]:
    ...
    y = 240
    return {'dim_left': pygame.Rect(center_x - 170, y, 60, 40), 'dim_value': pygame.Rect(center_x - 100, y, 200, 40), 'dim_right': pygame.Rect(center_x + 110, y, 60, 40), 'sdg_left': pygame.Rect(center_x - 170, y + 80, 60, 40), 'sdg_value': pygame.Rect(center_x - 100, y + 80, 200, 40), 'sdg_right': pygame.Rect(center_x + 110, y + 80, 60, 40), 'continue': pygame.Rect(center_x - 150, y + 180, 145, 45), 'back': pygame.Rect(center_x + 5, y + 180, 145, 45)}
...
def init_config(state: Dict[str, Any]) -> None:
    ...
    state['inputs'] = {'dimension': 5, 'sdg_id': 1}
    state['focusable'] = ['dim_left', 'dim_right', 'sdg_left', 'sdg_right', 'continue', 'back']
    state['focus_index'] = 4
    state['rects'] = _layout()
...
def draw(surface: pygame.Surface, state: Dict[str, Any]) -> None:
    ...
    rects = _layout()
    state['rects'] = rects
    draw_text(surface, 'Configuracion de cartones', (WINDOW_WIDTH // 2, 80), font_size=40, color=COLOR_PINE, center=True)
    mouse_pos = pygame.mouse.get_pos()
    hovered = {name: rect.collidepoint(mouse_pos) for (name, rect) in rects.items()}
    focused = get_focused(state)
    draw_text(surface, 'Dimension del carton:', (rects['dim_value'].centerx, rects['dim_value'].y - 30), font_size=22, color=COLOR_CHARCOAL, center=True)
    draw_button(surface, '<', rects['dim_left'], hovered=hovered['dim_left'], focused=focused == 'dim_left')
    draw_text(surface, f"{state['inputs']['dimension']} x {state['inputs']['dimension']}", rects['dim_value'].center, font_size=24, color=COLOR_CHARCOAL, center=True)
    draw_button(surface, '>', rects['dim_right'], hovered=hovered['dim_right'], focused=focused == 'dim_right')
    sdg_id = state['inputs']['sdg_id']
    sdg_name = get_sdg_name(sdg_id)
    draw_text(surface, 'Tema ODS:', (rects['sdg_value'].centerx, rects['sdg_value'].y - 30), font_size=22, color=COLOR_CHARCOAL, center=True)
    ...
    draw_text(surface, f'ODS {sdg_id}: {sdg_name}', rects['sdg_value'].center, font_size=18, color=COLOR_WHITE, center=True)
    draw_button(surface, '>', rects['sdg_right'], hovered=hovered['sdg_right'], focused=focused == 'sdg_right')
    draw_button(surface, 'Continuar', rects['continue'], hovered=hovered['continue'], focused=focused == 'continue')
    draw_button(surface, 'Volver', rects['back'], hovered=hovered['back'], focused=focused == 'back')
    if state.get('error_message'):
        draw_error_message(surface, state['error_message'], (WINDOW_WIDTH // 2, WINDOW_HEIGHT - 80), font_size=20)