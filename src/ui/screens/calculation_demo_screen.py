...
def _layout() -> Dict[str, pygame.Rect]:
    ...
    return {'trigger': pygame.Rect(center_x - 120, 240, 240, 45), 'back': pygame.Rect(center_x - 120, 310, 240, 45)}
...
def _build_engine_context() -> Dict[str, Any]:
    ...
    return make_engine_context(registry=registry, preconditions=[], dependency_checker=dependency_checker, impact_persistence=make_impact_persistence())
...
def _build_sample_payload() -> Dict[str, Any]:
    ...
    return {'actor_id': 'player1', 'operation_key': 'tombola', 'path_context': {'domain': 'tombola', 'path': 'card_coverage'}, 'input_payload': {'card': card, 'drawn_numbers': drawn_numbers}, 'ui_origin': 'calculation_demo'}
...
def _activate(state: Dict[str, Any], rect_name: str) -> str:
    ...
    if rect_name == 'trigger':
        demo_screen = state['calculation_demo_screen']
        result = calculation_screen.on_calculate_action(demo_screen, _build_sample_payload())
...
def draw(surface: pygame.Surface, state: Dict[str, Any]) -> None:
    ...
    rects = _layout()
    state['rects'] = rects
    draw_text(surface, 'Demo de calculo', (WINDOW_WIDTH // 2, 80), font_size=40, color=COLOR_PINE, center=True)
    draw_text(surface, 'Ejecuta un comando interno de suma de demostracion.', (WINDOW_WIDTH // 2, 140), font_size=22, color=COLOR_CHARCOAL, center=True)
    ...
    draw_button(surface, 'Ejecutar calculo', rects['trigger'], hovered=hovered['trigger'], focused=focused_control == 'calculate_button')
    draw_button(surface, 'Volver', rects['back'], hovered=hovered['back'], focused=focused_control == 'cancel_button')
    ...
    if result:
        if result.get('status') == 'success':
            ...
            color = (220, 80, 80)
        draw_text(surface, text, (WINDOW_WIDTH // 2, 420), font_size=22, color=color, center=True)
