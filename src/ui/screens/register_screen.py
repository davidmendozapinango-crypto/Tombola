...
def _draw_data_panel(
    ...
    for key, label in (("m", "Masculino"), ("f", "Femenino"), ("o", "Otro")):
        ...
        bg = (
            COLOR_PINE
            if selected
            else (COLOR_MOSS if hovered[f"gender_{key}"] else COLOR_WHITE)
        )
...
def _draw_validator_panel(surface: pygame.Surface, state: Dict[str, Any]) -> None:
    ...
    # Intro text
    intro_text = (
        "El juego requiere claves de acceso robustas conforme con los estándares "
        "gubernamentales para proteger la bitácora JUGADORES.bin contra intrusiones:"
    )
    ...
    suggestion_text = (
        f"{suggestion}, que cumple con las restricciones y se encriptará de forma "
        "simétrica antes de empaquetarse."
    )
