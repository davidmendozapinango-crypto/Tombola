...
def _layout() -> Dict[str, pygame.Rect]:
    ...
    right_w = 400
    panel_h = 515
    return {'player_id': pygame.Rect(left_x + 20, left_y + 95, left_w - 40, 34), 'full_name': pygame.Rect(left_x + 20, left_y + 160, left_w - 40, 34), 'gender_m': pygame.Rect(left_x + 20, left_y + 225, 80, 30), 'gender_f': pygame.Rect(left_x + 105, left_y + 225, 80, 30), 'gender_o': pygame.Rect(left_x + 190, left_y + 225, 70, 30), 'birthdate': pygame.Rect(left_x + 270, left_y + 225, left_w - 290, 30), 'state_left': pygame.Rect(left_x + 20, left_y + 290, 32, 32), 'state_value': pygame.Rect(left_x + 57, left_y + 290, left_w - 110, 32), 'state_right': pygame.Rect(left_x + left_w - 52, left_y + 290, 32, 32), 'access_key': pygame.Rect(left_x + 20, left_y + 355, left_w - 40, 34), 'confirm_key': pygame.Rect(left_x + 20, left_y + 420, left_w - 40, 34), 'login_link': pygame.Rect(left_x + 20, left_y + 545, 180, 28), 'register': pygame.Rect(left_x + 210, left_y + 540, left_w - 230, 40)}

def _draw_person_icon(surface: pygame.Surface, rect: pygame.Rect) -> None:
    """Draw a simple person outline icon."""
    (cx, cy) = rect.center
...
def _suggest_password() -> str:
    """Return a sample password that meets all criteria."""
    return 'Aura*2026'

def init_register(state: Dict[str, Any]) -> None:
    """Initialize registration screen state."""
    state['inputs'] = {'player_id': '', 'full_name': '', 'gender': '', 'birthdate': '', 'state_code': '', 'access_key': '', 'confirm_key': ''}
    state['focusable'] = ['player_id', 'full_name', 'gender_m', 'gender_f', 'gender_o', 'birthdate', 'state_left', 'state_right', 'access_key', 'confirm_key', 'login_link', 'register']
    state['focus_index'] = 0
    state['rects'] = _layout()
...
def _try_register(state: Dict[str, Any]) -> str:
    ...
    if access_key != confirm_key:
        ...
        return 'register'
    data = {'player_id': inputs.get('player_id', '').strip(), 'full_name': inputs.get('full_name', '').strip(), 'gender': inputs.get('gender', '').strip().lower(), 'birthdate': inputs.get('birthdate', '').strip(), 'state_code': inputs.get('state_code', '').strip().upper(), 'access_key': access_key}
...
def _draw_title(surface: pygame.Surface) -> None:
    """Draw the screen title and subtitle."""
    draw_text(surface, 'Registro de Jugador Acreditado', (WINDOW_WIDTH // 2, 55), font_size=32, color=COLOR_PINE, center=True)
    italic_font = get_font(14)
    italic_font.set_italic(True)
    subtitle = italic_font.render('Uniendo voluntades para alcanzar los Objetivos de Desarrollo Sostenible.', True, COLOR_CHARCOAL)
    rect = subtitle.get_rect(center=(WINDOW_WIDTH // 2, 85))
    surface.blit(subtitle, rect)

def _draw_data_panel(surface: pygame.Surface, state: Dict[str, Any], rects: Dict[str, pygame.Rect], hovered: Dict[str, bool], focused: str) -> None:
    """Draw the left player data panel."""
    left_x = 70
    left_y = 115
    left_w = 460
    ...
    draw_text(surface, 'Datos de Jugador', (panel_rect.x + 54, panel_rect.y + 22), font_size=18, color=COLOR_PINE, center=False)
    pygame.draw.line(surface, COLOR_SAGE_LIGHT, (panel_rect.x + 18, panel_rect.y + 60), (panel_rect.right - 18, panel_rect.y + 60), 1)
    draw_text(surface, 'CÉDULA DE IDENTIDAD (V- ...)', (rects['player_id'].x, rects['player_id'].y - 20), font_size=11, color=COLOR_CHARCOAL, center=False)
    draw_input(surface, state['inputs'].get('player_id', ''), rects['player_id'], focused=is_focused(state, 'player_id'), placeholder='Ej: 12345678')
    draw_text(surface, 'NOMBRE COMPLETO', (rects['full_name'].x, rects['full_name'].y - 20), font_size=11, color=COLOR_CHARCOAL, center=False)
    draw_input(surface, state['inputs'].get('full_name', ''), rects['full_name'], focused=is_focused(state, 'full_name'), placeholder='Ej: Camila Valentina Silva')
    draw_text(surface, 'GÉNERO / SEXO', (rects['gender_m'].x, rects['gender_m'].y - 20), font_size=11, color=COLOR_CHARCOAL, center=False)
    ...
    for (key, label) in (('m', 'Masculino'), ('f', 'Femenino'), ('o', 'Otro')):
        ...
        bg = COLOR_PINE if selected else COLOR_MOSS if hovered[f'gender_{key}'] else COLOR_WHITE
        pygame.draw.rect(surface, bg, rect, border_radius=15)
        border_color = COLOR_PINE if selected or hovered[f'gender_{key}'] else COLOR_SAGE_LIGHT
        pygame.draw.rect(surface, border_color, rect, width=1, border_radius=15)
        draw_text(surface, label, rect.center, font_size=11, color=COLOR_WHITE if selected else COLOR_CHARCOAL, center=True)
    draw_text(surface, 'FECHA DE NACIMIENTO', (rects['birthdate'].x, rects['birthdate'].y - 20), font_size=11, color=COLOR_CHARCOAL, center=False)
    draw_input(surface, state['inputs'].get('birthdate', ''), rects['birthdate'], focused=is_focused(state, 'birthdate'), placeholder='01/01/2000')
    draw_text(surface, 'ESTADO DE PROCEDENCIA (REGIÓN ORIGEN)', (rects['state_value'].x, rects['state_value'].y - 20), font_size=11, color=COLOR_CHARCOAL, center=False)
    ...
    draw_text(surface, f'{state_label} ({state_code})' if state_code else state_label, rects['state_value'].center, font_size=13, color=COLOR_CHARCOAL, center=True)
    draw_button(surface, '>', rects['state_right'], hovered=hovered['state_right'])
    draw_text(surface, 'CLAVE DE ACCESO (CRITERIO ODS)', (rects['access_key'].x, rects['access_key'].y - 20), font_size=11, color=COLOR_CHARCOAL, center=False)
    draw_input(surface, state['inputs'].get('access_key', ''), rects['access_key'], focused=is_focused(state, 'access_key'), mask=True, placeholder='Ej: Clave*25')
    draw_text(surface, 'CONFIRMAR CLAVE', (rects['confirm_key'].x, rects['confirm_key'].y - 20), font_size=11, color=COLOR_CHARCOAL, center=False)
    draw_input(surface, state['inputs'].get('confirm_key', ''), rects['confirm_key'], focused=is_focused(state, 'confirm_key'), mask=True, placeholder='Repita su clave')

def _draw_bottom_actions(surface: pygame.Surface, rects: Dict[str, pygame.Rect], hovered: Dict[str, bool], focused: str) -> None:
    """Draw the login link and register button below the data panel."""
    link_color = COLOR_PINE if hovered['login_link'] else COLOR_CHARCOAL
    draw_text(surface, '¿Ya estás registrado? Inicia Sesión', (rects['login_link'].x, rects['login_link'].y + 6), font_size=13, color=link_color, center=False)
    draw_button(surface, 'Generar Ciudadanía  →', rects['register'], hovered=hovered['register'], focused=focused == 'register')

def _draw_validator_panel(surface: pygame.Surface, state: Dict[str, Any]) -> None:
    """Draw the right password validator panel."""
    right_x = 550
    right_y = 115
    right_w = 400
    ...
    draw_text(surface, 'VALIDADOR DE CLAVE', (panel_rect.x + 54, panel_rect.y + 20), font_size=18, color=COLOR_WHITE, center=False)
    draw_text(surface, 'Criterios Algorítmicos Recursivos', (panel_rect.x + 54, panel_rect.y + 40), font_size=12, color=COLOR_MOSS, center=False)
    pygame.draw.line(surface, (76, 122, 79), (panel_rect.x + 18, panel_rect.y + 75), (panel_rect.right - 18, panel_rect.y + 75), 1)
    ...
    items = [('length_ok', 'Longitud Permitida', 'Debe tener exactamente de 6 a 10 caracteres.'), ('has_uppercase', 'Mezcla de Caracteres', 'Debe contener al menos una Mayúscula, una minúscula y un número.'), ('has_special', 'Carácter Especial Exclusivo', 'Debe incluir al menos uno de estos símbolos: * = % _'), ('no_long_run', 'Sin Repetitividad Secuencial', 'No se admiten más de 3 caracteres idénticos consecutivos.'), ('keys_match', 'Confirmacion de Clave', 'Ambas claves deben coincidir.', keys_match)]
    y = line_y + 10
    for (index, item) in enumerate(items):
        ...
        draw_text(surface, f'{index + 1}. {title}', (panel_rect.x + 58, y), font_size=13, color=COLOR_WHITE, center=False)
        ...
        for word in words:
            ...
            if font.size(test)[0] <= max_width:
                ...
                draw_text(surface, line, (panel_rect.x + 58, line_y), font_size=11, color=COLOR_MOSS, center=False)
                line = word
                line_y += 16
        if line:
            draw_text(surface, line, (panel_rect.x + 58, line_y), font_size=11, color=COLOR_MOSS, center=False)
    ...
    draw_text(surface, 'Sugerencia de Clave Válida:', (panel_rect.x + 18, panel_rect.bottom - 65), font_size=12, color=COLOR_WHITE, center=False)
    ...
    for line in suggestion_lines[:3]:
        draw_text(surface, line, (panel_rect.x + 18, line_y), font_size=11, color=COLOR_MOSS, center=False)
        line_y += 16

def draw(surface: pygame.Surface, state: Dict[str, Any]) -> None:
    """Render the redesigned registration screen."""
    ...
    if state.get('error_message'):
        draw_error_message(surface, state['error_message'], (WINDOW_WIDTH // 2, WINDOW_HEIGHT - 75), font_size=18)
