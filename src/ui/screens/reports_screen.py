...
from src.ui.common import draw_button, draw_error_message, draw_input, draw_message_panel, draw_text, wrap_text
_REPORT_SECTIONS = [('players', 'Jugadores y partidas'), ('top', 'TOP 5 jugadores'), ('frequency', 'Numeros mas frecuentes'), ('history', 'Historial de partidas')]
_SECTION_LABELS = {'players': 'Jugadores y partidas', 'top': 'TOP 5 jugadores por puntos', 'frequency': 'Numeros mas frecuentes', 'history': 'Historial de partidas'}

def _layout() -> Dict[str, pygame.Rect]:
    """Return the UI rectangles for the reports screen."""
    center_x = WINDOW_WIDTH // 2
    return {'date_start': pygame.Rect(center_x - 220, 90, 200, 32), 'date_end': pygame.Rect(center_x + 20, 90, 200, 32), 'apply': pygame.Rect(center_x - 100, 130, 90, 32), 'clear': pygame.Rect(center_x + 10, 130, 90, 32), 'players': pygame.Rect(60, 185, 190, 35), 'top': pygame.Rect(60, 225, 190, 35), 'frequency': pygame.Rect(60, 265, 190, 35), 'history': pygame.Rect(60, 305, 190, 35), 'back': pygame.Rect(WINDOW_WIDTH - 60 - 150, WINDOW_HEIGHT - 105, 150, 35), 'export': pygame.Rect(WINDOW_WIDTH - 60 - 150 - 155, WINDOW_HEIGHT - 105, 150, 35)}

def init_reports(state: Dict[str, Any]) -> None:
    """Initialize reports screen state."""
    state['inputs'] = {'date_start': '', 'date_end': ''}
    state['focusable'] = ['date_start', 'date_end', 'apply', 'clear', 'players', 'top', 'frequency', 'history', 'export', 'back']
    state['focus_index'] = 4
    state['rects'] = _layout()
...
def _filter_games_by_date(games: List[Dict[str, Any]], start_text: str, end_text: str) -> Tuple[List[Dict[str, Any]], Optional[str]]:
...
def _player_game_counts(players, games):
    ...
    for game in games:
        ...
        if pid in counts:
            counts[pid] += 1
    return [(player['player_id'], player['full_name'], counts.get(player['player_id'], 0)) for player in players]
...
def _top_players(players, games, limit=5):
    ...
    """
    player_info = {player['player_id']: (player.get('full_name', player['player_id']), player.get('state_code', '?')) for player in players}
    totals = Counter()
    game_counts = Counter()
    for game in games:
        pid = game.get('player_id')
    ...
    return [(pid, player_info.get(pid, (pid, '?'))[0], player_info.get(pid, (pid, '?'))[1], game_counts.get(pid, 0), points) for (pid, points) in sorted_totals]
...
def _build_players_report(players, games, date_start: str='', date_end: str='') -> str:
    ...
    lines.append('')
    lines.append(f"{'CEDULA':<15} {'JUGADOR':<28} {'ESTADO':<10} {'PARTIDAS JUGADAS':<20}")
    lines.append('-' * 80)
    if players:
...
def _build_top_report(players, games, date_start: str='', date_end: str='') -> str:
    ...
    if top:
        for (rank, (pid, name, state_code, game_count, points)) in enumerate(top, start=1):
            ...
            lines.append(f'{rank}. {name} - Region: {state_code} - {game_count} {partidas} - {points} ODS puntos')
    else:
...
def _build_frequency_report(games, date_start: str='', date_end: str='') -> str:
    ...
    lines.append('La aleatoriedad de la tombola se aproxima a la campana estadistica en sesiones largas. Los datos se actualizan dinamicamente.')
    lines.append('')
    lines.append('=' * 60)
    return '\n'.join(lines)

def _build_history_report(players, games, date_start: str='', date_end: str='') -> str:
    ...
    lines.append('')
    lines.append(f"{'FECHA':<12} {'CODIGO':<10} {'JUGADOR':<25} {'EST':<6} {'BASE':<6} {'TEMA ODS SORTEADO':<28} {'SORTEOS':<12} {'CARTON GANADOR':<18} {'PUNTAJE':<10}")
    lines.append('-' * 140)
    if games:
        for (index, game) in enumerate(games, start=1):
            ...
            lines.append(f'{date_text:<12} {log_code:<10} {player_name:<25} {state_code:<6} {base:<6} {sdg_name:<28} {sorteos_text:<12} {carton_text:<18} {puntaje:<10}')
    else:
        lines.append('No hay partidas registradas.')
    lines.append('')
    lines.append('=' * 60)
...
def _build_report_text(players, games, section: str, date_start: str='', date_end: str='') -> str:
...
def _export_reports(state: Dict[str, Any]) -> str:
    ...
    text = _build_report_text(state['players'], state['filtered_games'], section, state['inputs'].get('date_start', ''), state['inputs'].get('date_end', ''))
...
def _apply_date_filter(state: Dict[str, Any]) -> None:
    ...
    (games, error) = _filter_games_by_date(state['all_games'], state['inputs'].get('date_start', ''), state['inputs'].get('date_end', ''))
    if error:
...
def _draw_section_button(surface: pygame.Surface, label: str, rect: pygame.Rect, hovered: bool, focused: bool, selected: bool) -> None:
...
def _draw_players_report(surface, players, games, content_x, content_y, content_w):
    ...
    draw_text(surface, 'JUGADORES Y PARTIDAS REGISTRADOS', (content_x, content_y), font_size=18, color=COLOR_PINE)
    draw_text(surface, f'Registros totales: {len(players)}', (content_x + content_w - 140, content_y + 5), font_size=12, color=COLOR_CHARCOAL)
    columns = [('CEDULA', 140), ('JUGADOR', 250), ('ESTADO', 100), ('PARTIDAS JUGADAS', 140)]
    col_x = [content_x]
    ...
    for (idx, (label, width)) in enumerate(columns):
        x = col_x[idx] + 4
        draw_text(surface, label, (x, table_y + header_height // 2), font_size=11, color=COLOR_WHITE)
    table_y += header_height
    if not players:
        draw_text(surface, 'No hay jugadores registrados.', (content_x, table_y + 8), font_size=18)
        return
    for (pid, name, count) in _player_game_counts(players, games):
        ...
        for (idx, (value, width)) in enumerate(zip(row_values, [c[1] for c in columns])):
            ...
            for line in wrap_text(value, width - 8, font_size=11):
                draw_text(surface, line, (x, text_y), font_size=11, color=COLOR_CHARCOAL)
                break
        table_y += row_height
...
def _draw_top_report(surface, players, games, content_x, content_y, content_w):
    ...
    draw_text(surface, 'TOP 5: JUGADORES DESTACADOS', (content_x, content_y), font_size=22, color=COLOR_PINE)
    draw_text(surface, 'Lideres acumulados registrados en JUGADORES.bin', (content_x, content_y + 26), font_size=12, color=COLOR_CHARCOAL)
    y = content_y + 55
    top = _top_players(players, games)
    if not top:
    ...
    for (rank, (pid, name, state_code, game_count, points)) in enumerate(top, start=1):
        ...
        draw_text(surface, str(rank), circle_center, font_size=16, color=COLOR_CHARCOAL, center=True)
        draw_text(surface, name, (content_x + 55, y + 10), font_size=18, color=COLOR_CHARCOAL)
        partidas = 'partida' if game_count == 1 else 'partidas'
        draw_text(surface, f'Region: {state_code} · {game_count} {partidas}', (content_x + 55, y + 32), font_size=13, color=COLOR_CHARCOAL)
        draw_text(surface, str(points), (content_x + content_w - 70, y + 10), font_size=22, color=COLOR_PINE)
        draw_text(surface, 'ODS', (content_x + content_w - 35, y + 10), font_size=10, color=COLOR_CHARCOAL)
        draw_text(surface, 'PUNTOS', (content_x + content_w - 48, y + 32), font_size=10, color=COLOR_CHARCOAL)
...
def _draw_frequency_report(surface, games, content_x, content_y, content_w):
    ...
    draw_text(surface, 'Frecuencia de numeros (TOP 10)', (content_x, content_y), font_size=22, color=COLOR_PINE)
    y = content_y + 35
    gantt = _gantt_numbers(games)
    if not gantt:
    ...
    for (rank, (number, count)) in enumerate(gantt, start=1):
        draw_text(surface, f'#{rank}', (content_x, y + 2), font_size=14, color=COLOR_CHARCOAL)
        number_rect = pygame.Rect(content_x + number_offset, y, number_box_size, number_box_size)
        pygame.draw.rect(surface, COLOR_PINE, number_rect)
        draw_text(surface, str(number), number_rect.center, font_size=14, color=COLOR_WHITE, center=True)
        ...
        draw_text(surface, sorteos_text, (bar_x + bar_max_width + 10, y + 3), font_size=14, color=COLOR_CHARCOAL)
        y += row_height
    note_y = y + 10
    draw_text(surface, 'Nota del Algoritmo:', (content_x, note_y), font_size=14, color=COLOR_PINE)
    note_lines = wrap_text('La aleatoriedad de la tombola se aproxima a la campana estadistica en sesiones largas. Los datos se actualizan dinamicamente.', content_w, font_size=12)
    for line in note_lines:
        note_y += 18
        draw_text(surface, line, (content_x, note_y), font_size=12, color=COLOR_CHARCOAL)

def _draw_history_report(surface, players, games, content_x, content_y, content_w):
    """Draw the game history report as a table."""
    player_map = _player_lookup(players)
    draw_text(surface, 'HISTORIAL HISTORICO DE PARTIDAS (JUEGOS.BIN)', (content_x, content_y), font_size=18, color=COLOR_PINE)
    draw_text(surface, 'Bitacora de auditoria Federal del juego educativo', (content_x, content_y + 22), font_size=12, color=COLOR_CHARCOAL)
    total_text = f'Registros totales: {len(games)}'
    draw_text(surface, total_text, (content_x + content_w - 140, content_y + 5), font_size=12, color=COLOR_CHARCOAL)
    columns = [('FECHA', 65), ('CODIGO LOG', 60), ('JUGADOR', 125), ('ESTADO', 50), ('BASE N', 50), ('TEMA ODS SORTEADO', 125), ('SORTEOS REALIZADOS', 75), ('CARTON GANADOR', 85), ('PUNTAJE', 49)]
    col_x = [content_x]
    for (_, width) in columns[:-1]:
        col_x.append(col_x[-1] + width)
    ...
    table_y += header_height
    if not games:
        draw_text(surface, 'No hay partidas registradas.', (content_x, table_y + 8), font_size=18)
        return
    for (index, game) in enumerate(games, start=1):
        row_rect = pygame.Rect(content_x, table_y, content_w, row_height)
        pygame.draw.rect(surface, COLOR_WHITE, row_rect)
        ...
        carton_text = f'{carton} ({winning_sum})'
        puntaje = f"+{summary['total_points']}"
        row_values = [date_text, log_code, player_name, state_code, base, sdg_name, sorteos_text, carton_text, puntaje]
        for (idx, (value, width)) in enumerate(zip(row_values, [c[1] for c in columns])):
            x = col_x[idx] + 4
            text_y = table_y + row_height // 2
            for line in wrap_text(value, width - 8, font_size=11):
...
def _draw_report_content(surface: pygame.Surface, section: str, players, games, content_x: int, content_y: int, content_w: int) -> None:
...
def draw(surface: pygame.Surface, state: Dict[str, Any]) -> None:
    ...
    rects = _layout()
    state['rects'] = rects
    draw_text(surface, 'Reportes', (WINDOW_WIDTH // 2, 40), font_size=40, color=COLOR_PINE, center=True)
    draw_text(surface, 'Desde (YYYY-MM-DD):', (rects['date_start'].x, rects['date_start'].y - 20), font_size=16, color=COLOR_CHARCOAL)
    draw_input(surface, state['inputs'].get('date_start', ''), rects['date_start'], focused=is_focused(state, 'date_start'))
    draw_text(surface, 'Hasta (YYYY-MM-DD):', (rects['date_end'].x, rects['date_end'].y - 20), font_size=16, color=COLOR_CHARCOAL)
    draw_input(surface, state['inputs'].get('date_end', ''), rects['date_end'], focused=is_focused(state, 'date_end'))
    mouse_pos = pygame.mouse.get_pos()
    hovered = {name: rect.collidepoint(mouse_pos) for (name, rect) in rects.items()}
    focused = get_focused(state)
    draw_button(surface, 'Aplicar', rects['apply'], hovered=hovered['apply'], focused=focused == 'apply')
    draw_button(surface, 'Limpiar', rects['clear'], hovered=hovered['clear'], focused=focused == 'clear')
    selected = state.get('selected_report', 'all')
    for (section, label) in _REPORT_SECTIONS:
        _draw_section_button(surface, label, rects[section], hovered=hovered.get(section, False), focused=focused == section, selected=selected == section)
    content_x = 280
    content_y = 185
    ...
    _draw_report_content(surface, selected, players, games, content_x + 15, content_y + 15, content_w - 30)
    draw_button(surface, 'Exportar .txt', rects['export'], hovered=hovered['export'], focused=focused == 'export')
    draw_button(surface, 'Volver', rects['back'], hovered=hovered['back'], focused=focused == 'back')
    export_path = state.get('report_export_path')
    if export_path:
        draw_text(surface, f'Exportado: {Path(export_path).name}', (WINDOW_WIDTH // 2, 600), font_size=16, color=COLOR_PINE, center=True)
    if state.get('error_message'):
        draw_error_message(surface, state['error_message'], (WINDOW_WIDTH // 2, 625), font_size=20)
