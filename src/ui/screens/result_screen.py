...
def _layout() -> Dict[str, pygame.Rect]:
    ...
    y = 520
    return {'play_again': pygame.Rect(center_x - 150, y, 145, 45), 'menu': pygame.Rect(center_x + 5, y, 145, 45)}
...
def draw(surface: pygame.Surface, state: Dict[str, Any]) -> None:
    ...
    winning_card = session['main_card'] if winner == 'main' else session['complement_card']
    winning_sum = card_sum(winning_card)
    draw_text(surface, 'Resultado de la partida', (WINDOW_WIDTH // 2, 80), font_size=40, color=sdg_color, center=True)
    draw_text(surface, 'GANADOR', (WINDOW_WIDTH // 2, 160), font_size=48, color=COLOR_PINE, center=True)
    draw_text(surface, f'Carton ganador: {winner.upper()}', (WINDOW_WIDTH // 2, 230), font_size=28, color=COLOR_CHARCOAL, center=True)
    draw_text(surface, f'Suma de celdas del carton ganador: {winning_sum}', (WINDOW_WIDTH // 2, 290), font_size=24, color=COLOR_CHARCOAL, center=True)
    draw_text(surface, f'Tema ODS: {get_sdg_name(sdg_id)}', (WINDOW_WIDTH // 2, 350), font_size=24, color=COLOR_CHARCOAL, center=True)
    draw_text(surface, f"Numeros sorteados: {len(session.get('drawn_numbers', []))}", (WINDOW_WIDTH // 2, 410), font_size=22, color=COLOR_CHARCOAL, center=True)
    mouse_pos = pygame.mouse.get_pos()
    hovered = {name: rect.collidepoint(mouse_pos) for (name, rect) in rects.items()}
    focused = get_focused(state)
    draw_button(surface, 'Jugar de nuevo', rects['play_again'], hovered=hovered['play_again'], focused=focused == 'play_again')
