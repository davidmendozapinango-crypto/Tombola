"""
Pantalla de resultado de la partida (estilo funcional, no-OOP).

Descripción:
  Este módulo renderiza la pantalla de resultados al finalizar una partida.
  Muestra el cartón ganador, la suma de sus celdas, el tema ODS seleccionado y
  botones para volver al menú o jugar de nuevo. Todas las funciones aceptan y
  modifican el diccionario de `state` compartido por la aplicación.

Notas:
  - No modifica la lógica del juego; solo presenta información que viene
    almacenada en `state['session']`.
"""
...
def _layout() -> Dict[str, pygame.Rect]:
    """
    Devuelve los rectángulos (posiciones) usados por la interfaz de resultado.
...
def init_result(state: Dict[str, Any]) -> None:
    """
    Inicializa el estado necesario para la pantalla de resultados.
...
def handle_event(state: Dict[str, Any], event: pygame.event.Event) -> str:
    """
    Procesa un evento de Pygame y devuelve el nombre de la pantalla siguiente.

    Args:
        state (Dict[str, Any]): Estado global de la aplicación.
        event (pygame.event.Event): Evento recibido desde la cola de Pygame.

    Devuelve:
        str: Nombre de la pantalla a mostrar a continuación.
    """
...
def _activate(state: Dict[str, Any], name: str) -> str:
    """
    Ejecuta la acción asociada a una opción de la pantalla de resultados.

    Args:
        state (Dict[str, Any]): Estado global de la aplicación.
        name (str): Identificador del control activado ('play_again'|'menu').

    Devuelve:
        str: Nombre de la pantalla destino.
    """
    if name == "play_again":
        return "config"
    if name == "menu":
...
def draw(surface: pygame.Surface, state: Dict[str, Any]) -> None:
    """
    Dibuja (renderiza) la pantalla de resultados en el `surface` proporcionado.

    Args:
        surface (pygame.Surface): Superficie donde dibujar.
        state (Dict[str, Any]): Estado global que contiene la sesión y datos.

    Devuelve: None (mutación/lectura de `state`).
    """
