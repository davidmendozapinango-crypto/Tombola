"""
Pantalla de ingreso (Login) - panel educativo y seleccion de cuenta (no-OOP).

Descripción:
  Permite seleccionar un jugador registrado, introducir la clave y autenticar
  la sesión. Presenta un encabezado educativo y accesos para registro.
"""
...
def _layout() -> Dict[str, pygame.Rect]:
    """
    Devuelve los rectángulos (posiciones) usados en la pantalla de login.
...
def _draw_globe_icon(surface: pygame.Surface, rect: pygame.Rect) -> None:
    """Dibuja un icono de globo dentro de una insignia."""
    (cx, cy) = rect.center
    radius = 10
...
def _draw_key_icon(surface: pygame.Surface, rect: pygame.Rect) -> None:
    """Dibuja un icono simple de llave (clave)."""
    (cx, cy) = rect.center
...
def _draw_shield_icon(surface: pygame.Surface, rect: pygame.Rect) -> None:
    """Dibuja un icono simple de escudo usado en el boton de autenticacion."""
    (cx, cy) = rect.center
    pygame.draw.polygon(
        surface,
        COLOR_WHITE,
...
def _draw_header(surface: pygame.Surface, state: Dict[str, Any]) -> None:
    """Dibuja el encabezado superior con el mensaje educativo del simulador."""
    ...
    draw_text(
        surface, "Tómbola ODS", (75, 34), font_size=22, color=COLOR_WHITE, center=False
    )
...
def _draw_tabs(
    ...
    """Dibuja las pestañas (login / registrar) sobre la interfaz."""
...
def _draw_left_panel(
    ...
    """Dibuja el panel izquierdo con controles de selección de jugador y clave."""
...
def init_login(state: Dict[str, Any]) -> None:
    """
    Inicializa el estado para la pantalla de login: entradas, jugadores
    disponibles y controles enfocables.
    """
    state["inputs"] = {"key": ""}
...
def _selected_player(state: Dict[str, Any]) -> Dict[str, Any]:
    """Devuelve el registro del jugador actualmente seleccionado en el UI."""
    players = state.get("players", [])
    index = state.get("selected_player_index", 0)
...
def _change_player(state: Dict[str, Any], direction: int) -> None:
    """Recorre (cicla) la lista de jugadores registrados según `direction`."""
    players = state.get("players", [])
    if not players:
        return
    index = state.get("selected_player_index", 0)
...
def _select_player(state: Dict[str, Any], index: int) -> None:
    """Selecciona un jugador por su índice dentro de la lista cargada."""
    players = state.get("players", [])
...
def _try_login(state: Dict[str, Any]) -> str:
    """
    Valida las credenciales introducidas y realiza la autenticación.

    En caso de éxito llama a `login` y devuelve 'menu', en caso contrario
    establece un mensaje de error y permanece en 'login'.
    """
...
def handle_event(state: Dict[str, Any], event: pygame.event.Event) -> str:
    """Procesa eventos de Pygame para la pantalla de login y devuelve la
    pantalla siguiente.

    Maneja clics del ratón y eventos de teclado (TAB, ENTER, ESCAPE) y
    actualiza `state` en consecuencia.
    """
...
def draw(surface: pygame.Surface, state: Dict[str, Any]) -> None:
    """
    Renderiza la pantalla de ingreso: encabezado, selector de jugador,
    campo de clave y botones.
    """
