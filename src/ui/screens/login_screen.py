...
def _layout() -> Dict[str, pygame.Rect]:
    ...
    return {'tab_login': pygame.Rect(WINDOW_WIDTH // 2 - 220, 95, 220, 40), 'tab_register': pygame.Rect(WINDOW_WIDTH // 2, 95, 220, 40), 'panel': pygame.Rect(left_x, left_y, panel_w, panel_h), 'selector_left': pygame.Rect(left_x + 20, left_y + 135, 32, 32), 'selector_value': pygame.Rect(left_x + 57, left_y + 135, 316, 32), 'selector_right': pygame.Rect(left_x + 378, left_y + 135, 32, 32), 'key': pygame.Rect(left_x + 20, left_y + 225, 390, 38), 'register_link': pygame.Rect(left_x + 20, left_y + 285, 390, 28), 'login': pygame.Rect(left_x + 20, left_y + 335, 390, 45)}
...
def _draw_shield_icon(surface: pygame.Surface, rect: pygame.Rect) -> None:
    ...
    (cx, cy) = rect.center
    pygame.draw.polygon(surface, COLOR_WHITE, [(cx, cy - 10), (cx + 9, cy - 4), (cx + 9, cy + 4), (cx, cy + 10), (cx - 9, cy + 4), (cx - 9, cy - 4)])
...
def _draw_header(surface: pygame.Surface, state: Dict[str, Any]) -> None:
    ...
    draw_text(surface, 'SIMULADOR EDUCATIVO', (75, 18), font_size=11, color=COLOR_MOSS, center=False)
    draw_text(surface, 'Tómbola ODS', (75, 34), font_size=22, color=COLOR_WHITE, center=False)

def _draw_tabs(surface: pygame.Surface, rects: Dict[str, pygame.Rect], hovered: Dict[str, bool]) -> None:
    ...
    draw_text(surface, 'Identidad Configuracion', (login_rect.centerx, login_rect.y + 12), font_size=13, color=COLOR_WHITE, center=True)
    draw_text(surface, '(Login)', (login_rect.centerx, login_rect.y + 27), font_size=11, color=COLOR_MOSS, center=True)
    ...
    draw_text(surface, 'Registrar Jugador (Nuevo)', register_rect.center, font_size=13, color=COLOR_PINE, center=True)

def _draw_left_panel(surface: pygame.Surface, state: Dict[str, Any], rects: Dict[str, pygame.Rect], hovered: Dict[str, bool], focused: str) -> None:
    ...
    draw_text(surface, 'Ingreso Jugador', (panel_rect.x + 56, panel_rect.y + 24), font_size=22, color=COLOR_PINE, center=False)
    ...
    pygame.draw.line(surface, COLOR_SAGE_LIGHT, (panel_rect.x + 20, panel_rect.y + 85), (panel_rect.right - 20, panel_rect.y + 85), 1)
    draw_text(surface, 'ELEGIR JUGADOR REGISTRADO', (panel_rect.x + 20, panel_rect.y + 105), font_size=12, color=COLOR_PINE, center=False)
    ...
    draw_text(surface, label, rects['selector_value'].center, font_size=14, color=COLOR_CHARCOAL, center=True)
    draw_button(surface, '>', rects['selector_right'], hovered=hovered['selector_right'])
    draw_text(surface, 'CLAVE DE SEGURIDAD REGISTRADA', (panel_rect.x + 20, panel_rect.y + 195), font_size=12, color=COLOR_PINE, center=False)
    draw_input(surface, state['inputs'].get('key', ''), rects['key'], focused=is_focused(state, 'key'), mask=True, placeholder='Introduzca su clave de acceso')
    link_color = COLOR_PINE if hovered['register_link'] else COLOR_CHARCOAL
    draw_text(surface, '¿Falta tu usuario?', (panel_rect.x + 20, panel_rect.y + 290), font_size=13, color=COLOR_CHARCOAL, center=False)
    draw_text(surface, 'Registrar Jugador', (panel_rect.x + 160, panel_rect.y + 290), font_size=13, color=link_color, center=False)
    draw_button(surface, 'Autenticar y Entrar', rects['login'], hovered=hovered['login'], focused=focused == 'login')
    shield_rect = pygame.Rect(rects['login'].x + 18, rects['login'].centery - 10, 20, 20)
...
def init_login(state: Dict[str, Any]) -> None:
    ...
    state['focusable'] = ['selector_left', 'selector_right', 'key', 'register_link', 'login', 'tab_register']
    state['focus_index'] = 0
    state['rects'] = _layout()
...
def draw(surface: pygame.Surface, state: Dict[str, Any]) -> None:
    ...
    if state.get('error_message'):
        draw_error_message(surface, state['error_message'], (WINDOW_WIDTH // 2, WINDOW_HEIGHT - 70), font_size=18)
