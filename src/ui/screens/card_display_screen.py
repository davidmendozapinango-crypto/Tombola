...
def _layout() -> Dict[str, pygame.Rect]:
    ...
    return {'play': pygame.Rect(center_x - 160, WINDOW_HEIGHT - 90, 150, 45), 'back': pygame.Rect(center_x + 10, WINDOW_HEIGHT - 90, 150, 45)}
...
def _draw_preview_card(surface: pygame.Surface, dimension: int, pattern: set, top_left: Any, cell_size: int, title: str, sdg_color: Any) -> None:
    ...
    for row in range(dimension):
        for col in range(dimension):
            ...
            if is_figure:
                draw_text(surface, str(sequence), rect.center, font_size=max(10, cell_size // 2), color=COLOR_WHITE, center=True)
                sequence += 1
...
def draw(surface: pygame.Surface, state: Dict[str, Any]) -> None:
    ...
    draw_text(surface, f'Vista previa - {sdg_name}', (WINDOW_WIDTH // 2, 40), font_size=36, color=COLOR_PINE, center=True)
    draw_text(surface, f'Figura del carton: tipo {card_type}', (WINDOW_WIDTH // 2, 85), font_size=22, color=COLOR_CHARCOAL, center=True)
    ...
    _draw_preview_card(surface, dimension, main_pattern, (80, 140), cell_size, 'Principal (figura a completar)', COLOR_PINE)
    _draw_preview_card(surface, dimension, complement_pattern, (WINDOW_WIDTH - 80 - dimension * cell_size, 140), cell_size, 'Complemento (figura a completar)', COLOR_PINE)
    image_y = 420
    ...
    focused = get_focused(state)
    draw_button(surface, 'Jugar', rects['play'], hovered=hovered['play'], focused=focused == 'play')
