"""Pantalla que muestra la vista previa de la figura SDG antes de jugar."""
...
def _layout() -> Dict[str, pygame.Rect]:
    """Rectángulos usados por la pantalla (botones Play/Back)."""
...
def _load_card_image(card_type: str, is_main: bool) -> Optional[pygame.Surface]:
    """Cargar la imagen PNG de la figura para el tipo de carta seleccionado.

    Devuelve `None` si la imagen no existe o no puede cargarse.
    """
...
def _draw_preview_card(
    ...
    """Dibujar una vista previa de la tarjeta indicando las celdas de la figura.

    Las celdas pertenecientes a la figura se rellenan y se numeran en orden
    secuencial para facilitar la visualización.

    Args:
        surface: Superficie pygame donde se dibuja.
        dimension: Tamaño N de la cuadrícula.
        pattern: Conjunto de coordenadas (fila,col) que forman la figura.
        top_left: Tupla (x,y) coordenada superior izquierda del área de dibujo.
        cell_size: Tamaño en pixeles de cada celda.
        title: Texto que se muestra encima de la previsualización.
        sdg_color: Color temático asociado al ODS (no usado para numeración aquí).
    """
...
def init_card_display(state: Dict[str, Any]) -> None:
    """Inicializar la pantalla de vista previa de tarjeta en el estado de la app."""
    state["inputs"] = {}
...
def handle_event(state: Dict[str, Any], event: pygame.event.Event) -> str:
    """Procesar eventos Pygame y devolver el nombre de la pantalla siguiente."""
...
def draw(surface: pygame.Surface, state: Dict[str, Any]) -> None:
    """Renderizar la pantalla de vista previa de tarjeta en la superficie.

    Muestra la figura principal y complementaria, imágenes si existen y los
    botones para jugar o volver.
    """
